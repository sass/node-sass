...
function getArgument(name) {
  ...
  for (var i = 0; i < flags.length; i++) {
    if (flags[i].indexOf(name) === 0 && flags[i].substr(name.length, 1) === '=') {
      flag = flags[i];
      break;
    }
  }
  ...
  return flag.substr(flag.indexOf('=') + 1);
}
...
function getBinaryPath() {
  ...
  if (!path.isAbsolute(binaryPath)) {
    var absoluteBinaryPath = getAbsolutePath(binaryPath);
    if (absoluteBinaryPath) {
...
/**
 * Try to transform the supplied relative path into an absolute path.
 *
 * @param {String} relativePath
 * @return {String} absolute path if found, otherwise empty string
 * @api private
 */
...
function getAbsolutePath(relativePath) {
  ...
  var directoryCount = process.cwd().split(path.sep).length - 1;
