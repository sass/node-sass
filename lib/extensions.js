...
function getHumanEnvironment(env) {
  ...
  var moduleVersion = parseInt(parts[2])
  if (!runtime) {
    runtime = 'Unsupported runtime (' + moduleVersion + ')';
  } else if (moduleVersion > 14 && moduleVersion < 45) {
    // Warn for iojs support
    console.warn('Node-Sass no longer provides binaries for ' + runtime);
  } else if (moduleVersion === 11) {
    console.warn('Node will be dropping support October 2016, please upgrade soon ' + runtime);
  } else if (moduleVersion > 14 && moduleVersion < 47) {
    console.warn('Node-Sass no longer provides binaries for ' + runtime);
  }
