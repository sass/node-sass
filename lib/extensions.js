...
function isSupportedEnvironment(platform, arch, abi) {
  return (
    ...
    false !== getElectronSupport(platform, abi)
  );
}

/**
 * Suport for Electron is (currently) partial so check node versions against win32
 * Note: Only called after a *kind of* true result for isSupportedEnvironment
 *
 * @param  {string} platform - The name of the OS platform(darwin, win32, etc...)
 * @param  {string} abi - The Node Application Binary Interface
 * @return {Boolean} True, if node-sass supports the Electron platform, false otherwise
 * @api private
 */
function getElectronSupport(platform, abi) {
  if (
    new Array("70", "73", "75", "76").indexOf(abi || process.versions.modules) && 
    false == new Array("win32").indexOf(platform || process.platform)
  )
    return false;
