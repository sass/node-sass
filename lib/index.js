...
module.exports.render = function(opts, cb) {
  ...
  try {
    payload = this.renderSync(opts);
  } catch (e) {
    cb.bind(options.context)(e, null);
    return;
  }

  cb.bind(options.context)(null, payload);
};
  ...
  if (options.functions) {
    ...
    Object.keys(functions).forEach(function(signature) {
      ...
      options.functions[cb.signature] = function() {
        var args = Array.prototype.slice.call(arguments),
          bridge = args.pop(),
          isAsync = !!bridge.success,
          done = isAsync ? bridge.success : bridge;
