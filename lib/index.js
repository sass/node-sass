...
/**
 * Returns function to be called when executing a custom importer
 *
 * @param {Object} target
 * @param {Object} options
 * @api private
...
function establishImporter(target, options) {
  return function(file, prev, bridge) {
    function done(data) {
      bridge.success(data);
    }

    var result = target.call(options.context, file, prev, done);
...
/**
 * Returns function to be called when synchronously executing a custom importer
 *
 * @param {Object} target
 * @param {Object} options
 * @api private
...
function establishImporterSync(target, options) {
  return function(file, prev) {
    var resultCb;
    function done(data) {
      resultCb = data;
    }
    ...
    var result = target.call(options.context, file, prev, done);
    result = (typeof resultCb !== 'undefined') ? resultCb : result;
  ...
  if (importer) {
    if (Array.isArray(importer)) {
      importer.forEach(function(subject, index) {
        options.importer[index] = establishImporter(subject, options);
      });
    } else {
      options.importer = establishImporter(importer, options);
    }
  }
...
module.exports.renderSync = function(options) {
  ...
  if (importer) {
    if (Array.isArray(importer)) {
      importer.forEach(function(subject, index) {
        options.importer[index] = establishImporterSync(subject, options);
      });
    } else {
      options.importer = establishImporterSync(importer, options);
    }
  }
