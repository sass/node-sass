...
var cwd = process.cwd();
var isTTY = process.stdin.isTTY || process.env.isTTY;
...
function checkCallback(argv) {
  if (argv.help) { return true; }
  if (!argv._.length && isTTY) {
    return false;
  } 
}

// A hack to show custom messages instead of the whole function body.
checkCallback.toString = function() {
  return '\n  Use --help for usage and available options';
};  
...
function run(options, emitter) {
  ...
  if (options.watch) {
    ...
    watch(watchDir, function(file) {
      emitter.emit('warn', chalk.grey('=> changed: ') + chalk.blue(file));
